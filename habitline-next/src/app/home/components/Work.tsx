  ...
  return (
    <section className="relative" id="usecase">
      ...
      <div className="relative z-10 container rounded-2xl bg-white object-cover p-8.5 text-center lg:mx-auto lg:p-25" style={{ backgroundImage: `url(${vector10.src})` }}>
        <span className="border-default-200 text-ink mb-3.5 inline-block rounded-full border bg-white px-5 py-1.5 text-sm font-medium">Fits every lifestyle</span>

        <h2 className="text-ink mb-8 text-2xl leading-[1.1] font-medium tracking-normal md:mb-12.5 md:text-4xl lg:text-5xl">
          Adapted for the way
        ...
        <nav className="mb-7.5 flex flex-wrap justify-center gap-2.5 md:gap-5" aria-label="Tabs" role="tablist">
          {usecaseTabsData.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={`hs-tab-active:border-default-800 hs-tab-active:bg-default-100 hs-tab-active:text-ink bg-default-100 border-default-200 text-default-500 hover:bg-default-100 inline-flex items-center gap-x-2.5 rounded-xl border px-5 py-2.5 text-base font-medium transition-all md:text-lg ${tab.isActive ? 'active' : ''}`}
              id={tab.id}
        ...
        <div className="mt-6 md:mt-12.5">
          <p className="text-ink mb-2.5 text-sm font-medium">And for every kind of daily rhythm</p>
          <div className="flex flex-wrap justify-center gap-2.5 md:gap-6.5">
            {hashtagsData.map((tag, idx) => (
              <span key={idx} className="text-ink text-sm font-medium transition-colors">
                {tag}
              </span>
            ))}
          </div>
