      ...
      <div className="nav-sticky navbar fixed inset-x-0 top-0 z-120 w-full">
        <div className="container py-3.5 md:py-5 lg:py-7.5">
          <div className="flex w-full items-center justify-between rounded-lg bg-white md:rounded-2xl lg:justify-center lg:bg-transparent">
            <div className="flex min-w-[175px] items-center justify-center rounded-[20px] bg-white p-1.5 lg:shadow-lg">
              <Link href="/" onClick={() => setCurrentHash('')} className="flex h-8.75 items-center justify-center md:h-12.5">
                ...
                <ul id="navbar" className="flex items-center">
                    ...
                    return (
                      <li key={index}>
                        <Link
                          ...
                          className={`flex items-center justify-center rounded-2xl px-7 py-4 text-sm font-medium transition-all ${isActive ? 'bg-default-200 text-ink' : 'text-default-700 hover:bg-default-200 hover:text-ink'}`}
                        >
                          {item.label}
                        </Link>
                      </li>
                  ...
                  {/* <li className="hs-dropdown relative inline-flex [--trigger:hover]">
                    <button
                      ...
                      type="button"
                      className="hs-dropdown-toggle hover:bg-default-200 hover:text-ink flex items-center justify-center gap-1 rounded-2xl px-7 py-4 text-sm transition-all focus:outline-hidden disabled:pointer-events-none disabled:opacity-50"
                      aria-haspopup="menu"
                      aria-expanded="false"
                      aria-label="Dropdown"
                    >
                        ...
                        {dropdownItems.map((item, index) => {
                          ...
                          return (
                            <Link
                              ...
                              className={`flex items-center gap-x-3.5 rounded-lg px-2.5 py-1.5 text-sm transition-all ${isActive ? 'bg-default-200 text-ink font-medium' : 'text-default-600 hover:text-ink hover:bg-default-200'}`}
                            >
                              {item.label}
                            </Link>
                          )
            ...
            <div className="hs-accordion-content hidden w-full overflow-hidden ps-5 pb-4 transition-[height]">
              {dropdownItems.map((item, index) => (
                <Link
                  ...
                  className="text-default-600 hover:text-ink hover:bg-default-200 flex items-center gap-x-3.5 rounded-lg px-2.5 py-1.5 text-base focus:outline-hidden"
                >
                  {item.label}
                </Link>
              ))}
